...
1. コミット（メッセージは `<type>: <概要>` 形式）
2. PR 作成（base・ラベル・`Closes #XXX` を本文に記載）

途中で止まってよいのは「不明点がある場合」のみ。
その場合は PR Comments に質問を書くこと。
...
4. 【Cursor】Issue URL をデスクトップアプリで渡して実装依頼
```
...
1. 上記フォーマットを受け取ったら自動で Issue 草稿（.md）を出力する
2. 草稿には「再現手順・期待動作・影響ファイル候補・完了定義」を含める
3. Naoya に「GitHub Issue を作成 → Cursor に URL を渡す」手順を案内する
