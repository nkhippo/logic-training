...
- [ ] **多言語対応チェック**（UIに関わる仕様の場合のみ）
  - ユーザー可視の文字列（ラベル・placeholder・メッセージ等）は ja/en 両方の文言が明記されているか
  - 文字数・幅などの数値制約は、言語特性（全角/半角・プロポーショナル字幅）を考慮して言語別に検討されているか
  - 言語切替時の state 挙動（値のリセット有無・言語依存の有無）が定義されているか
