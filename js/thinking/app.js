...
async function submitThinkingStep(stepIdx, mode) {
  ...
  try {
    ...
    // user_coreの更新（レベル2〜4の定義ステップ）
    if (result.userCore && result.userCore.trim().length > 0 && mode !== 'answer') {
      prob.userCore = result.userCore.trim();
    }
...
// ── 振り返り機能（役割D2・D3） ────────────────────────
function skipThinkingReflection() {
  if (thinkingSt.problem) thinkingSt.problem.reflectionStep = -1;
...
function showThinkingReflectionArea(prob) {
  if (prob.reflectionStep === -1) return;
  const l = L[prob.lang];
  const isEN = prob.lang === 'en';
  const reflectionEl = document.getElementById('thinking-reflection');
  ...
  qArea.innerHTML = `
    ...
    <div class="action-bar no-print" style="margin-top:8px;gap:8px;">
      ...
      <button class="btn btn-sm" style="background:var(--bg2);color:var(--text2);"
              onclick="skipThinkingReflection()">
...
function openThinkingGuide() {
  ...
  const isEN = thinkingSt.lang === 'en';
  const guidePath = isEN ? 'guide/en/thinking-overview.md' : 'guide/thinking-overview.md';
  fetch(guidePath)
    .then(r => r.text())
    .then(text => {
...
async function loadThinkingPast() {
  ...
  thinkingSt.past=rows.map(mapThinkingPastRow).reverse();
...
async function syncThinkingPast(prob) {
  ...
  const entry = {
    ...
    user_core: prob.userCore || '',
    theme: prob.theme || '',
    persona_snapshot: JSON.stringify(getPersonaSnapshot() || {}),
    lang: prob.lang,
  };
  try {
    await gasPost(entry);
