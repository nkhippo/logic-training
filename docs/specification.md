...
左寄せ（上から順）と右寄せの2カラム。思考トレーニング（`specification-thinking.md` §3-1）と同一パターン。

| 位置 | 要素 | 内容 |
|---|---|---|
| 左 | サービス名 | "thinkgrindai" |
| 左 | ページタイトル | "論理トレーニング" / "Logic Training \| thinkgrindai" |
| 左 | ガイド | "ガイド？" → ガイドモーダル |
| 左 | 思考トレーニングへのリンク | `思考トレーニング →` |
| 右 | プロフィール | ペルソナ設定モーダル（`js/shared/13-persona-modal.js`） |
...
| 2.4 | 2026-05-24 | ヘッダーUIを2カラム化。左：ガイド？・他アプリへのリンク。右：プロフィール・言語（モーダル選択）。思考トレーニングでもプロフィールを利用可能に。 |
| 2.5 | 2026-05-25 | 積み上げタブを廃止（UI・js/17-tsumiaage.js・ガイド・GAS tsumiaage 連携を削除）。論理トレーニングは4タブ構成に。 |
