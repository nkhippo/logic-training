# thinkgrindai ドキュメント

## コア（Claude.ai Projects Knowledge 推奨）

| ファイル | 内容 |
|---------|------|
| [../CLAUDE.md](../CLAUDE.md) | Claude・Cursor 共通ルール |
| [PROJECT_CONTEXT.md](PROJECT_CONTEXT.md) | ビジョン・背景・戦略 |
| [DEVELOPMENT_POLICY.md](DEVELOPMENT_POLICY.md) | 開発ルール・ロール責務 |

Knowledge 更新手順: [PROJECTS_KNOWLEDGE_UPDATE_GUIDE.md](PROJECTS_KNOWLEDGE_UPDATE_GUIDE.md)

## 運用・セットアップ

| ファイル | 内容 |
|---------|------|
| [TASK_TRACKER_URL.md](TASK_TRACKER_URL.md) | Google Sheets タスク管理 |
| [setup/COMPREHENSIVE_SETUP_GUIDE.md](setup/COMPREHENSIVE_SETUP_GUIDE.md) | 開発体制セットアップ総合ガイド |
| [setup/2025-01-24_setup-summary.md](setup/2025-01-24_setup-summary.md) | セットアップ完了サマリー |
| [setup/GITHUB_PROJECTS_SETUP.md](setup/GITHUB_PROJECTS_SETUP.md) | GitHub Projects 設定 |
| [setup/GOOGLE_SHEETS_SETUP.md](setup/GOOGLE_SHEETS_SETUP.md) | Google Sheets 設定 |
| [ai-context/FILE_MAP.md](ai-context/FILE_MAP.md) | **AI向けファイルマップ**（Claude・Cursor 用） |
| [setup/OBSIDIAN_SETUP.md](setup/OBSIDIAN_SETUP.md) | Obsidian Vault 設定（`~/Documents/Obsidian/ThinkGrindAi`） |

## 技術・仕様

| ファイル | 内容 |
|---------|------|
| [architecture.md](architecture.md) | アーキテクチャ |
| [dev-flow.md](dev-flow.md) | ブランチ運用・リリースフロー |
| [gas-column-headers.md](gas-column-headers.md) | GAS シート列定義 |
| [requirements.md](requirements.md) / [specification.md](specification.md) | Logic Training |
| [requirements-thinking.md](requirements-thinking.md) / [specification-thinking.md](specification-thinking.md) | Thinking Training |

## Cursor 指示書

- [cursor-instructions/](cursor-instructions/) — 機能ごとの実装指示書

## Wiki（新規参入者向け）

正本は [.github/wiki/](../.github/wiki/) です。`main` へ push すると GitHub Wiki に自動同期されます。

Automation bridge test note: README minimal update applied.
