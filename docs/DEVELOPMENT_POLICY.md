...
### 実装中に Issue 本文が更新されたとき

仕様の正本は常に **GitHub Issue の最新本文**。Naoya が実装中に Issue を追記・修正した場合:

| 誰 | やること |
|---|---|
| **Naoya** | Issue 更新後、Cursor に「Issue を更新した」と伝える（または Issue URL を再送する） |
| **Cursor** | Issue を再取得する（`gh issue view` または Issue URL）。追記された完了定義・実装範囲を確認し、既存実装と矛盾があれば Naoya に確認してから続行 |
| **Cursor** | PR 作成前に、Issue 本文のチェックリストと実装内容が一致していることを確認する |

Cursor 向けの詳細手順は `.cursor/rules/dev-flow.mdc` の「実装中に Issue が更新されたとき」を参照。
